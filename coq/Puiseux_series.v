(* $Id: Puiseux_series.v,v 1.468 2013-09-05 11:07:58 deraugla Exp $ *)
        ...
        destruct st as [| st].
         ...
         apply Nbar.mul_lt_mono_pos_r in Hlt₂.
          destruct k as [k| ]; [ idtac | inversion Hlt₂ ].
            ...
            rewrite Nat.div_0_l in Hs; simpl in Hs.
             ...
             destruct (Nbar.lt_dec 0 (fin (Pos.to_nat (nz_comden nz))))
              ...
              destruct (Nbar.lt_dec 0 1) as [Hlt₄| Hge₄].
               ...
               rewrite <- Nat.mul_succ_l.
               destruct
                (Nbar.lt_dec 0 (fin (S st * Pos.to_nat (nz_comden nz))))
                as [Hlt₅| Hge₅].
                destruct (lt_dec 0 (Pos.to_nat (nz_comden nz)))
                 as [Hlt₆| Hge₆].
                 destruct (Nbar.lt_dec 0 (fin (S st))) as [Hlt₇| Hge₇].
                ...
                exfalso; apply Hge₅.
                constructor.
                remember (Pos.to_nat (nz_comden nz)) as p.
                destruct p.
                 exfalso; symmetry in Heqp; revert Heqp; apply Pos2Nat_ne_0.
           ...
           apply lt_S_n in H1.
           subst n0 m.
           remember (series_tail (nz_terms nz)) as x.
           remember (stretch_series fld (nz_comden nz) x) as y.
           subst x.
           remember
            (series_nth_fld fld (S k * Pos.to_nat (nz_comden nz))
               (series_pad_left fld (Pos.to_nat (nz_comden nz)) y)) as s
            eqn:Hs .
           subst y.
           unfold series_tail in Hs; simpl in Hs.
           unfold series_nth_fld, stretch_series in Hs.
           rewrite Nat.add_comm in Hs.
           rewrite <- Nat.mul_succ_l in Hs.
           unfold series_pad_left in Hs.
           simpl in Hs.
           rewrite Nat.add_comm in Hs.
           rewrite <- Nat.mul_succ_l in Hs.
           rewrite Hst in Hs.
           simpl in Hs.
           rewrite Nat.sub_0_r in Hs.
           rewrite <- Nat.mul_succ_l in Hs.
           rewrite Nat.add_comm in Hs.
           rewrite <- Nat.mul_succ_l in Hs.
           destruct
            (Nbar.lt_dec (fin (S k * Pos.to_nat (nz_comden nz)))
               (fin (S st * Pos.to_nat (nz_comden nz))))%Nbar 
            as [Hlt₃| Hge₃].
            destruct
             (lt_dec (S k * Pos.to_nat (nz_comden nz))
                (Pos.to_nat (nz_comden nz))) as [Hlt₄| Hge₄].
             exfalso; apply gt_not_le in Hlt₄; apply Hlt₄.
             simpl.
             apply le_plus_l.

             simpl in Hs.
             rewrite minus_plus in Hs.
             rewrite Nat.mod_mul in Hs.
              simpl in Hs.
              rewrite Nat.div_mul in Hs.
               unfold series_nth_fld in Hs.
               simpl in Hs.
               destruct (Nbar.lt_dec (fin k) (fin st)) as [Hlt₅| Hge₅].
                subst s.
                unfold stretch_series, series_head; simpl.
                unfold series_nth_fld; simpl.
                rewrite Hst; simpl.
                rewrite Nat.add_0_r.
                ...
                destruct (Nbar.lt_dec (fin (S k * x)) (fin x))
                 as [Hlt₆| Hge₆].
                 inversion_clear Hlt₆.
                 apply gt_not_le in H.
                 exfalso; apply H, le_plus_l.

                 rewrite fld_add_ident.
                 destruct (Nbar.lt_dec (fin (S k)) (fin (S st)))
                  as [Hlt₇| Hge₇].
                  reflexivity.
