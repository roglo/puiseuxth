...
Definition Pos_lcm (a b : positive) : positive :=
  match Z.lcm (Z.pos a) (Z.pos b) with
  | Z.pos p => p
  | _ => 1%positive (* fallback; shouldn't happen since a,b > 0 *)
  end.

(*
Definition Pos_lcm (a b : positive) : positive :=
  match N.lcm (Npos a) (Npos b) with
  | N0 => 1%positive (* fallback; shouldn't happen since a,b > 0 *)
  | Npos p => p
  end.
*)

Definition Q_add (x y : Q) :=
  let d := Pos_lcm (Qden x) (Qden y) in
  let n :=
    (Z.pos d * Qnum x / Z.pos (Qden x) +
     Z.pos d * Qnum y / Z.pos (Qden y))%Z
  in
  n # d.
...
Definition Q_add (x y : Q) :=
  ...
  let n :=
    ((Qnum x * QDen y + Qnum y * QDen x) *
       Z.pos d / Z.pos (Qden x * Qden y))%Z
  in
  n # d.
*)

Compute (let x := 1 # 6 in let y := 1 # 10 in (Qplus x y, Q_add x y)).
Compute (let x := 1 # 2 in let y := 1 # 2 in (Qplus x y, Q_add x y)).
Compute (let x := 1 # 2 in let y := 3 # 4 in (Qplus x y, Q_add x y)).

(*
...
Qplus = λ x y : Q, Qnum x * QDen y + Qnum y * QDen x # Qden x * Qden y
*)

Theorem N_pos_mul : ∀ a b, N.pos (a * b) = (N.pos a * N.pos b)%N.
Proof. easy. Qed.

Theorem Pos_lcm_mul_mono_l : ∀ n m p : positive,
  Pos_lcm (p * n) (p * m) = (p * Pos_lcm n m)%positive.
Proof.
intros.
progress unfold Pos_lcm.
do 2 rewrite Pos2Z.inj_mul.
rewrite Z.lcm_mul_mono_l.
rewrite Z.abs_eq; [ | easy ].
remember (Z.lcm (Z.pos n) (Z.pos m)) as a eqn:Ha.
symmetry in Ha.
destruct a; [ | easy | ]. {
  apply Z.lcm_eq_0 in Ha; cbn.
  now destruct Ha.
}
specialize (Z.lcm_nonneg (Z.pos n) (Z.pos m)) as H.
now rewrite Ha in H.
Qed.
...
Theorem Q_mul_add'_distr_l : ∀ x y z, x * (Q_add y z) = Q_add (x * y) (x * z).
Proof.
intros.
progress unfold Q_add.
progress unfold Qmult.
cbn.
f_equal; [ | now rewrite Pos_lcm_mul_mono_l ].
destruct x as (xn, xd).
destruct y as (yn, yd).
destruct z as (zn, zd).
cbn.
destruct yn as [| yn| yn]. {
  cbn.
  rewrite Z.mul_0_r; cbn.
  rewrite Pos_lcm_mul_mono_l.
  rewrite Pos2Z.inj_mul.
  destruct zn as [| zn| zn]. {
    cbn.
    now rewrite Z.mul_0_r.
  } {
    destruct xn as [| xn| xn]; [ easy | | ]. {
      cbn.
      do 5 rewrite Pos2Z.inj_mul.
      rewrite <- Z.mul_assoc.
      rewrite Zdiv_mult_cancel_l; [ | easy ].
      rewrite (Z.mul_comm (Z.pos xn)).
      rewrite (Z.mul_comm _ (_ * _)).
      rewrite (Z.mul_comm (Z.pos zn)).
      rewrite <- Z.mul_assoc.
      rewrite (Z.divide_div_mul_exact _ _ (Z.pos xn)); [ | easy | ]. 2: {
        apply Z.divide_mul_r.
        progress unfold Pos_lcm.
        remember (Z.lcm _ _) as a eqn:Ha.
        symmetry in Ha.
        destruct a as [| a| a]. {
          apply Z.lcm_eq_0 in Ha.
          now destruct Ha.
        } {
          rewrite <- Ha.
          apply Z.divide_lcm_r.
        }
        specialize (Z.lcm_nonneg (Z.pos yd) (Z.pos zd)) as H.
        now rewrite Ha in H.
      }
